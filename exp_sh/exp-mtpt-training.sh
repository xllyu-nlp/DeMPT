...
# running workspace
model_save_dir=$CODES/workspace/mtpt-model
...
CUDA_VISIBLE_DEVICES=$device deepspeed --master_port 29519 \
 $CODES/train.py \
    --num_train_epochs 4 \
    --train_path $data_dir/train.self_ctx.src.$src \
    --dev_path $data_dir/valid.self_ctx.src.$src \
...
if [ -f "$model_save_dir/model-$start.pt" ] && [ ! -f "$model_save_dir/$start/$dev_set.$src-$tgt.tran" ]; then
    ...
    CUDA_VISIBLE_DEVICES=$device \
        $python $CODES/inference.py \
                --input $data_dir/$dev_set.self_ctx.src.$src \
                --ptm $CKPT \
                --batch_size 2 \
                --keys_type_num 0 \
                --re_encoding 0 \
